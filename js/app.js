// Josephites Global Multi-Device Realtime Cloud Engine v3.6
...
function appendMessage(msg, scroll = true) {
  ...
  // Action buttons (Edit & Delete for Sender within 5 min / Admin Delete for any message)
  let actionButtonsHtml = '';
  if (msg.senderId !== 'system' && msg.senderId !== 'system_ai') {
    let showEdit = isSent && isWithin5Min && msg.type === 'TEXT';
    let showDelete = (isSent && isWithin5Min) || isAdmin;

    if (showEdit || showDelete) {
      actionButtonsHtml = `
        <div class="msg-actions">
          ${showEdit ? `<button class="msg-action-btn" onclick="editMessage('${msg.id}')" title="Edit (within 5 mins)">✏️ Edit</button>` : ''}
          ${showDelete ? `<button class="msg-action-btn delete" onclick="deleteMessage('${msg.id}')" title="${isAdmin && !isSent ? 'Admin Delete' : 'Delete (within 5 mins)'}">🗑️ Delete</button>` : ''}
        </div>
      `;
    }
  }
