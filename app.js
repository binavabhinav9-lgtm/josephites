// Josephites Global Multi-Device Realtime Cloud Engine v3.9
...
// Helper to check if current user is the author of a message
function isMessageSender(msg) {
  if (!msg) return false;
  if (msg.senderId && myUserId && msg.senderId === myUserId) return true;
  if (msg.senderName && currentUserName && msg.senderName.trim().toLowerCase() === currentUserName.trim().toLowerCase()) return true;
  return false;
}
  ...
  return [
    {
      ...
      content: "✨ Welcome to **Josephites**! Connect with fellow Josephites in real-time. (Tip: Hold or tap ⋮ on any message to edit, delete, or react).",
...
function handleIncomingCloudEvent(event) {
  ...
  if (event.type === 'NEW_MESSAGE') {
    ...
    const target = messages.find(m => m.id === event.messageId);
    if (target) {
      target.content = event.newContent;
      target.isEdited = true;
      saveStoredMessages();
      if (target.channelId === currentChannel) {
        renderCurrentChat();
      }
    }
    ...
    if (event.senderId !== myUserId && event.channelId === currentChannel) {
...
function handleUserTypingInput() {
  ...
  if (now - lastTypingBroadcast > 2000) {
    ...
    broadcastToCloud("josephites/cloud/typing", {
      type: 'USER_TYPING',
      senderId: myUserId,
...
const actionSheetPreview = document.getElementById('actionSheetPreview');
const sheetEditBtn = document.getElementById('sheetEditBtn');
const sheetDeleteBtn = document.getElementById('sheetDeleteBtn');
const sheetDeleteText = document.getElementById('sheetDeleteText');
const editMsgModalBackdrop = document.getElementById('editMsgModalBackdrop');
...
function renderMembers() {
  ...
  membersList.innerHTML = members.map(m => {
    ...
    const isMe = m.id === myUserId || (m.name && m.name.toLowerCase() === (currentUserName || '').toLowerCase());
...
// ========================================================
// TOUCH-AND-HOLD / ACTION SHEET CONTROLLER
// ========================================================
function openActionSheet(msgId) {
  const msg = messages.find(m => m.id === msgId);
  if (!msg) return;

  selectedActionMsgId = msgId;
  const isSender = isMessageSender(msg);
  const isAdmin = userRole === 'ADMIN';
  const isWithin5Min = (Date.now() - msg.timestamp) <= (5 * 60 * 1000);

  // Set message preview
  if (msg.type === 'VOICE') actionSheetPreview.innerText = '🎙️ Voice Note';
  else if (msg.type === 'IMAGE') actionSheetPreview.innerText = '📷 Photo Attachment';
  else if (msg.type === 'POLL') actionSheetPreview.innerText = `📊 Poll: ${msg.pollData?.question || ''}`;
  else actionSheetPreview.innerText = msg.content || '';

  // Configure Edit Button (Sender within 5 mins & Text message)
  const canEdit = isSender && isWithin5Min && (msg.type === 'TEXT' || !msg.type);
  sheetEditBtn.style.display = canEdit ? 'flex' : 'none';

  // Configure Delete Button (Sender within 5 mins OR Admin anytime)
  const canDelete = (isSender && isWithin5Min) || isAdmin;
  sheetDeleteBtn.style.display = canDelete ? 'flex' : 'none';
  if (sheetDeleteText) {
    sheetDeleteText.innerText = isAdmin && !isSender ? "Admin Delete (Anytime)" : "Delete (Within 5 mins)";
  }
...
function triggerSheetEdit() {
  const msgId = selectedActionMsgId;
  closeActionSheet();
  if (msgId) editMessage(msgId);
}

function triggerSheetDelete() {
  const msgId = selectedActionMsgId;
  closeActionSheet();
  if (msgId) deleteMessage(msgId);
}

function quickReactFromSheet(emoji) {
  const msgId = selectedActionMsgId;
  closeActionSheet();
  if (msgId) reactToMessage(msgId, emoji);
}

function copyMessageTextFromSheet() {
  const msg = messages.find(m => m.id === selectedActionMsgId);
  closeActionSheet();
  if (msg && msg.content) {
...
// Touch & Hold Long-Press Binder with Movement Threshold Tolerance
function bindLongPress(element, msgId) {
  let pressTimer = null;
  let startX = 0;
  let startY = 0;
  let lastTap = 0;
...
// ========================================================
// CUSTOM IN-APP MESSAGE EDIT MODAL (100% RELIABLE)
// ========================================================
function editMessage(msgId) {
  ...
  if (!isSender || !isWithin5Min) {
    return alert("You can only edit your own messages within 5 minutes of sending.");
  }

  editingMsgId = msgId;
  editMsgInput.value = msg.content || '';
  editMsgModalBackdrop.classList.remove('hidden');
  setTimeout(() => editMsgInput.focus(), 150);
}

function closeEditModal() {
  editMsgModalBackdrop.classList.add('hidden');
  editingMsgId = null;
}
...
function saveEditedMessage() {
  ...
  if (!msg) return closeEditModal();

  const newContent = editMsgInput.value.trim();
  if (!newContent) return alert("Message content cannot be empty.");
  ...
  broadcastToCloud("josephites/cloud/message_edit", {
    type: 'EDIT_MESSAGE',
...
function deleteMessage(msgId) {
  ...
  if (!isAdmin && (!isSender || !isWithin5Min)) {
    return alert("You can only delete your own messages within 5 minutes of sending.");
  }

  const confirmPrompt = isAdmin && !isSender ? 
    "Admin: Are you sure you want to delete this member's message?" : 
  ...
  if (confirm(confirmPrompt)) {
    ...
    broadcastToCloud("josephites/cloud/message_delete", {
      type: 'DELETE_MESSAGE',
      messageId: msgId
    });
  }
...
function appendMessage(msg, scroll = true) {
  const isSent = isMessageSender(msg);
  ...
  if (msg.type === 'VOICE' && msg.voiceData) {
    contentHtml = `
      <div class="voice-player">
        <button class="voice-btn" onclick="event.stopPropagation(); playVoiceNote('${msg.id}', '${msg.voiceData}')">▶</button>
      ...
      <div class="poll-card">
        ...
        <div>
          ${msg.pollData.options.map((opt, idx) => {
            ...
            return `
              <button class="poll-option-btn" onclick="event.stopPropagation(); votePoll('${msg.id}', ${idx})">
                <span>${opt.text}</span>
  ...
  row.innerHTML = `
    ...
    <div class="message-bubble" id="bubble-${msg.id}" title="Tap or hold to Edit, Delete or React">
      ${msg.senderId !== 'system' ? `<span class="bubble-menu-trigger" onclick="event.stopPropagation(); openActionSheet('${msg.id}')">⋮</span>` : ''}
      ...
      <div class="reactions-bar" id="rx-${msg.id}">
        ${(msg.reactions || []).map(r => `<span class="reaction-pill" onclick="event.stopPropagation(); reactToMessage('${msg.id}', '${r.emoji}')">${r.emoji} ${r.count}</span>`).join('')}
      </div>
    </div>
  `;
  ...
  // Bind Long-Press (Touch & Hold)
  const bubble = row.querySelector(`#bubble-${msg.id}`);
  if (bubble && msg.senderId !== 'system') {
